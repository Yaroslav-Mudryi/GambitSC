...
contract PaalBet is Ownable, ReentrancyGuard {
  ...
  function buyFromAMMWithEthAndReferrer (
    ...
    if (msg.value == 0) {
      IERC20(collateral).safeTransferFrom(msg.sender, address(this), expectedPayout);
      _approveTokenIfNeeded(collateral, SportsAMM, expectedPayout);
    }
    ISportsAMM(SportsAMM).buyFromAMMWithEthAndReferrer{value: msg.value}(
      market,
      position,
      amount,
      expectedPayout,
  ...
  function buyFromAMMWithDifferentCollateral (
    ...
    IERC20(collateral).safeTransferFrom(msg.sender, address(this), expectedPayout);
    _approveTokenIfNeeded(collateral, SportsAMM, expectedPayout);
    ISportsAMM(SportsAMM).buyFromAMMWithDifferentCollateral(
      market,
      position,
      amount,
      expectedPayout,
  ...
  function buyFromParlayWithEth(
    ...
    if (msg.value == 0) {
      IERC20(collateral).safeTransferFrom(msg.sender, address(this), _sUSDPaid);
      _approveTokenIfNeeded(collateral, ParlayMarketsAMM, _sUSDPaid);
    }
    
    IParlayMarketsAMM(ParlayMarketsAMM).buyFromParlayWithEth{value: msg.value}(
      _sportMarkets,
      _positions,
      _sUSDPaid,
      _additionalSlippage,
  ...
  function buyFromParlayWithDifferentCollateralAndReferrer(
    ...
    IParlayMarketsAMM(ParlayMarketsAMM).buyFromParlayWithDifferentCollateralAndReferrer(
      _sportMarkets,
      _positions,
      _sUSDPaid,
      _additionalSlippage,
